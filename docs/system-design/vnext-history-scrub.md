...
the module was pruned "40 → 12 packages". It was 9 when checked and is 8 now: the last one to
go was the dead root package (`uiembed.go`, `uiembed_embed.go`), which existed only for an
`embed_ui` build tag on the deleted `cmd/evidra-api`. The 12 was measured mid-prune and never
re-derived, which is the same failure the commit-count rule above describes: a figure quoted in
prose without the command that produces it becomes a

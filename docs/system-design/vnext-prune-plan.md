...
Either of two outcomes, not both:

- Gates A and C turn green as specified — which needs an arm that clears 15/16 with
  enforcement actually firing, plus a real operational server and a human read of one
  summary.
- Or the plan is revised to lower those bars to what the cheap arms demonstrably do
  (Gate A's numbers suggest that revision is the likely path), and §43's precondition
  is rewritten in the same commit as the first deletion.

Until one of those is on record, the prune is deliberately not executed, and this
